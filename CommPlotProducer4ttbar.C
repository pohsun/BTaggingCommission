...
{ 
  ...
  TH1D* evtnowrt                = new TH1D("evtnowrt",  	    "evtnowrt", 	     1,   0,1);
  
  // --------------------------------------Histograms declaration -----------------------------------------//
  
  AddHistottbar("nEvt",           "number evts",                1,   0,   1, syst);
  AddHistottbar("nEvtNoWrt",      "number evts no wrt",          1,   0,   1, syst);
  ...
  AddHistottbar("nEvt_afterJetSel",           "number evts",                1,   0,   1, syst);
  AddHistottbar("nEvtNoWrt_afterJetSel",      "number evts no wrt",          1,   0,   1, syst);
  AddHistottbar("njet_afterJetSel",           "number of jets",	        10,-0.5, 9.5, syst); //[ALPHA]
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) 
  {
    ...
    if(!computeWeights){ 
        cout<<" Evt "<<jentry<<" out wrt, pu: "<<puWgtNom<<endl;
     continue;
    }
    ...
    //cout<<"[ALPHA] Evt: "<<jentry<<" checking seletion"<<endl;
    ...
    if(!passsel)  continue;
    //cout<<"[ALPHA] --> Pass event seletion"<<endl;
    ...
    if(fillCommissioningHistograms)
    {
        ...
        evtnowrt->Fill(0);
        FillHistottbar_intFromMap("nEvt",               datatype, 0 ,0                                ,ww);
        FillHistottbar_intFromMap("nEvtNoWrt",          datatype, 0 ,0                                ,1);
    ...
    for (unsigned int ijet = 0; ijet < thettbarselector_.theSelJetColl.size(); ijet++) 
    {
      ...
      float cvsB     = CTag_Jet_CvsB[newJetIndex]; //ALPHA
      float cvsL     = CTag_Jet_CvsL[newJetIndex]; //ALPHA

      //cout<<">> [INFO] evt "<<jentry<<", j "<<newJetIndex<<", CvsL: "<<cvsL<<", CvsB: "<<cvsB<<endl;
	...
	for (int itrk=Jet_nFirstTrack[newJetIndex]; itrk<Jet_nLastTrack[newJetIndex] ; itrk++)
	  ...
	  //if (Track_nHitAll[itrk]>=8)           passNhit=true;  // relax for hip
	  //if (Track_nHitPixel[itrk]>=2)         passPix= true;  // relax for hip
	  if (Track_nHitAll[itrk]>=0)           passNhit=true;
	  if (Track_nHitPixel[itrk]>=0)         passPix= true;
	...
	if (n_sv>0)
      ...
      if( fillCommissioningHistograms)
      {
        ...
        //cout<<">> [INFO] evt "<<jentry<<", j "<<newJetIndex<<", CvsL: "<<cvsL<<", CvsB: "<<cvsB<<endl; //ALPHA
      }
    ...
    if(fillCommissioningHistograms)
    {
        FillHistottbar_intFromMap("njet",               datatype, 0 ,thettbarselector_.theSelJetColl.size()  , ww); //ALPHA
    ...
    if(isTTbarSelForSF == true)
    {
        ...
        FillHistottbar_intFromMap("nEvt_afterJetSel",                           datatype, 0 ,0                                ,ww);
        FillHistottbar_intFromMap("nEvtNoWrt_afterJetSel",                      datatype, 0 ,0                                ,1);
        FillHistottbar_intFromMap("njet_afterJetSel",                           datatype, 0 ,thettbarselector_.theSelJetColl.size()  , ww);
